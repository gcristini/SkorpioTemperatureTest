...
#include "cyapicallbacks.h"
    ...
    ***************************************/
    ...
    const UART_PC_UART_INIT_STRUCT UART_PC_configUart =
    {
        ...
        (uint8) UART_PC_UART_RX_BREAK_WIDTH
    };
    ...
    *******************************************************************************/
    void UART_PC_UartInit(const UART_PC_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            UART_PC_rxBufferSize  =         config->rxBufferSize;

            /* Set TX direction internal variables */
            UART_PC_txBuffer      =         config->txBuffer;
            UART_PC_txDataBits    = (uint8) config->dataBits;
            UART_PC_txBufferSize  =         config->txBufferSize;
            ...
            UART_PC_UART_RX_CTRL_REG = UART_PC_GET_UART_RX_CTRL_MODE(config->stopBits)              |
                                        ...
                                        UART_PC_GET_UART_RX_CTRL_BREAK_WIDTH(config->breakWidth);
...
void UART_PC_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: UART_PC_UartSendBreakBlocking
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    void UART_PC_UartSendBreakBlocking(uint32 breakWidth)
    {
        uint32 txCtrlReg;
        uint32 txIntrReg;

        /* Disable all UART TX interrupt source and clear UART TX Done history */
        txIntrReg = UART_PC_GetTxInterruptMode();
        UART_PC_SetTxInterruptMode(0u);
        UART_PC_ClearTxInterruptSource(UART_PC_INTR_TX_UART_DONE);

        /* Store TX CTRL configuration */
        txCtrlReg = UART_PC_TX_CTRL_REG;

        /* Set break width */
        UART_PC_TX_CTRL_REG = (UART_PC_TX_CTRL_REG & (uint32) ~UART_PC_TX_CTRL_DATA_WIDTH_MASK) |
                                        UART_PC_GET_TX_CTRL_DATA_WIDTH(breakWidth);

        /* Generate break */
        UART_PC_TX_FIFO_WR_REG = 0u;

        /* Wait for break completion */
        while (0u == (UART_PC_GetTxInterruptSource() & UART_PC_INTR_TX_UART_DONE))
        {
        }
        ...
        UART_PC_TX_CTRL_REG = txCtrlReg;
        UART_PC_SetTxInterruptMode(txIntrReg);
    }
